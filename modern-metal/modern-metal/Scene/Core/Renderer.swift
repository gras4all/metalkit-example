...
class Renderer: NSObject, MTKViewDelegate {
    ...
    static func buildScene(device: MTLDevice, vertexDescriptor: MDLVertexDescriptor) -> SceneModel {
        ...
        let light3 = Light(worldPosition: float3( 0, -5, -5), color: float3(0.3, 0.3, 0.3))
        scene.lights = [ light0, light1, light2, light3 ]

        let pot = Node(name: "Pot")
        let potMaterial = Material()
        let potBaseColorTexture = try? textureLoader.newTexture(name: "teapot_baseColor",
                                                                scaleFactor: 1.0,
                                                                bundle: nil,
                                                                options: options)
        potMaterial.baseColorTexture = potBaseColorTexture
        potMaterial.specularPower = 100
        potMaterial.specularColor = float3(0.8, 0.8, 0.8)
        ...
        pot.mesh = try! MTKMesh.newMeshes(asset: bobAsset, device: device).metalKitMeshes.first!

        scene.rootNode.children.append(pot)
    ...
    func update(_ view: MTKView) {
        ...
        if let bob = scene.nodeNamed("Pot") {
